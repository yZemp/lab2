...
def model_nonviene(t, V0, L, C):
    R = 2_000
    omega2 = 1 / (L * C)
    gamma = R / (2 * L)
    beta = np.sqrt(np.power(gamma, 2) - omega2)
...
def interp_nonviene(x, y, yerr, func = model_nonviene):
    my_cost = cost.LeastSquares(x, y, yerr, func)
    m = Minuit(my_cost, 6, 37e-3, 100e-9)
    m.limits["V0"] = (0, +np.inf)
    m.limits["L"] = (0, +np.inf)
    m.limits["C"] = (0, +np.inf)
    m.migrad()
...
def interp_2(x, y, yerr, func = model_2):
    my_cost = cost.LeastSquares(x, y, yerr, func)
    m = Minuit(my_cost, 10, 37e-3, 10e-9, 1) # DO NOT TOUCH !!!
    m = Minuit(my_cost, 10, 30e-3, 10e-9, 1) # Final
    m.limits["V0"] = (0, +np.inf)
    m.limits["L"] = (0, +np.inf)
    m.limits["C"] = (0, +np.inf)
    m.migrad()
...
def main():
    ...
    for i in range(aleph):
        ...
        if not i:
            ...
            lnsp = np.linspace(0.000_01, .0005, 10_000)
            # plt.plot(lnsp, model(lnsp, *m1.values), label = "$V(t) = V_0 (1 - e^{-t/\\tau})$", c = "#a515d5")
            # plt.plot(lnsp, model_nonviene(lnsp, *m1.values), label = "$V(t)$", c = "#a515d5")
    ...
    plt.hlines([Cavg, Cavg - Cerr, Cavg + Cerr], - 10, len(Cs) + 10, label = f"$C = ({Cavg * 10_000_000:.4f} \pm {Cerr * 10_000_000:.4f})x10^{-7}$", linestyles = "dotted", colors = "#a00101")
