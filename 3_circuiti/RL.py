...
temp_gen = clear_arr(data["2Agen [V]"].to_numpy()) / 2
absH2 = temp_ind / temp_gen
temp_ind_err = clear_arr(data["ind err [V]"].to_numpy())
temp_gen_err = clear_arr(data["gen err [V]"].to_numpy())
absH2err = np.sqrt(np.power(temp_ind_err / temp_gen, 2) + np.power((temp_ind * temp_gen_err) / np.power(temp_gen, 2), 2))
...
def main():
    ...
    plt.plot([], [], ' ', label = f"$L = (${m2.values[2] * 1e9:.1f} $\pm$ {m2.errors[2] * 1e9:.1f}$)x10^{-9}$")
    ...
    plt.plot([], [], ' ', label = f"$L = (${m4.values[2] * 1e9:.1f} $\pm$ {m4.errors[2] * 1e9:.1f}$)x10^{-9}$")
