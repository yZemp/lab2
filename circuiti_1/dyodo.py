...
def main():
    ...
    lnsp = np.linspace(potenziale[0] - 0.01, potenziale[-1] + 0.01, 10_000)
    axes[0].plot(lnsp, model(lnsp, *m2.values), label = "Legge", c = "#a515d5")

    axes[0].plot([], [], ' ', label = f"$\\chi^2_v$: {(m2.fval / m2.ndof):.3f}, P-value: {1. - chi2.cdf(m2.fval, df = m2.ndof):.4f}")
    axes[0].plot([], [], ' ', label = f"g = ({m2.values[0]:.2f} $\pm$ {m2.errors[0]:.2f}")
    ...
    lnsp = np.linspace(potenziale[0] - 0.01, potenziale[-1] + 0.01, 10_000)
    axes[1].plot(lnsp, model(lnsp, *m2.values), label = "Legge", c = "#a515d5")
    ...
    axes[1].plot([], [], ' ', label = f"g = ({m2.values[0]:.2f} $\pm$ {m2.errors[0]:.2f}")
