...
sheet_name = "tre_(ohm_serie)"
url = f"https://docs.google.com/spreadsheets/d/{sheet_id}/gviz/tq?tqx=out:csv&sheet={sheet_name}"
data = pd.read_csv(url)

corrente = data["Corrente (muA)"].to_numpy()[::-1] / 1000_000
corrente = data["Corrente (muA)"].to_numpy() / 1000_000
corrente = clear_arr(corrente)
sigma_corrente = data["Sigma corrente (muA)"].to_numpy()[::-1] / (np.sqrt(12) * 1000_000)
sigma_corrente = data["Sigma corrente (muA)"].to_numpy() / (np.sqrt(12) * 1000_000)
...
def main():
    ...
    plt.plot([], [], ' ', label = f"$Req$ = ({m2.values[0] / 1000_000:.2f} $\pm$ {m2.errors[0] / 1000_000:.2f})MOhm")
